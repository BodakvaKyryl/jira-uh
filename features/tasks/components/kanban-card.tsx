    ...
    <div className="mb-1.5 space-y-3 rounded bg-white p-2.5 shadow-sm">
      <div className="flex items-start justify-between gap-x-2">
        ...
        <TaskActions id={task.$id} projectId={task.projectId}>
          <MoreHorizontalIcon className="strike-1 size-[18px] shrink-0 text-neutral-700 transition hover:opacity-75" />
        </TaskActions>
      </div>
      <DottedSeparator />
      ...
      <div className="flex items-center gap-x-1.5">
        ...
        />
        <span className="flex text-xs font-medium">{task.project.name}</span>
      </div>
    </div>
  );
};
